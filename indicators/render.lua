--[[
  ...
  Projection via eqgfx.project (raw pixels + a reliable in-front-of-camera
...
local function spawn_ground(sp)
  return sp.X(), sp.Y(), sp.Z() - settings.data.groundOffset
end

-- Draw one active cast's area / line.
---@param a ActiveCast
function R.draw_active(dl, a)
  ...
  if cat == 'enemy'  and not S.showEnemies  then return end
  if cat == 'friend' and not S.showFriendly then return end
  if cat == 'self'   and not S.showSelf     then return end
  ...
  if not g then
    if S.genericMarker and S.showAoE then
  ...
  if tt == TT.DirectionalCone then
    if not S.showCones then return end
  ...
  elseif tt == TT.Beam then
    if not S.showBeams then return end
  ...
  elseif CASTER_CENTERED[tt] then
    if not S.showAoE then return end
    fill_and_outline(dl, cx, cy, cz, circle_pts(cx, cy, cz, radius, SEGMENTS), true, fillCol, lineCol)

  elseif TARGET_CENTERED[tt] then
    if not S.showTargetAoE then return end
    ...
    if not S.showLines then return end
    local tsp = a.targetID and mq.TLO.Spawn(a.targetID) or (a.isSelf and mq.TLO.Target or nil)
    if tsp and tsp() and tsp.X() then
      local tx, ty, tz = spawn_ground(tsp)
      local ax, ay, ai = eqgfx.project(cx, cy, cz)
      local bx, by, bi = eqgfx.project(tx, ty, tz)
      if ai and bi then dl:AddLine(ImVec2(ax, ay), ImVec2(bx, by), u32(S.colors.line), 3.0) end
    end
  end
end

-- Cyan reference ring around the player (toggled from the menu / /aering).
function R.draw_debug_ring(dl)
  local S = settings.data
  local me = mq.TLO.Me
  local x, y, z = me.X(), me.Y(), me.Z()
  if x then
    fill_and_outline(dl, x, y, z, circle_pts(x, y, z, S.debugRadius, SEGMENTS),
                     true, u32(S.colors.debugFill), u32(S.colors.debugLine))
  end
end
