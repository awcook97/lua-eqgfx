...
local function color(label, t, k)
  ...
  if changed then t[k] = to_color(colorVal); mark() end
end
...
function M.draw(caps)
  ...
  if show then
    ...
    if ImGui.CollapsingHeader('Names') then
      ...
      if cfg.name.shadow then color('  Shadow color', cfg.colors, 'nameShadow') end
    ...
    if ImGui.CollapsingHeader('Cast bar') then
      ...
      if cfg.castbar.showIcon then slideri('  Icon size', cfg.castbar, 'iconSize', 10, 40) end
      check('Show spell name', cfg.castbar, 'showSpellName')
      check('Show time remaining', cfg.castbar, 'showTime')
      if cfg.castbar.showTime then check('  Show total time too', cfg.castbar, 'showTotal') end
    ...
    if ImGui.CollapsingHeader('Buffs') then
      ...
      if ImGui.Button('+ Whitelist') and filterInput ~= '' then
        table.insert(cfg.buffs.whitelist, filterInput); filterInput = ''; mark()
      end
      ImGui.SameLine()
      if ImGui.Button('+ Blacklist') and filterInput ~= '' then
        table.insert(cfg.buffs.blacklist, filterInput); filterInput = ''; mark()
      end
      ...
      local keys = {}
      for k in pairs(cfg.buffs.overrides) do keys[#keys + 1] = k end
      table.sort(keys)
      for _, k in ipairs(keys) do
        ...
        if ImGui.TreeNode(k) then
          ...
          if ch then override.scale = v; mark() end
          local pv, pch = ImGui.SliderInt('Priority##' .. k, override.priority or 0, -10, 10)
          if pch then override.priority = pv; mark() end
          local hv, hch = ImGui.Checkbox('Hide##' .. k, override.hide or false)
          if hch then override.hide = hv; mark() end
          if ImGui.Button('Remove##' .. k) then cfg.buffs.overrides[k] = nil; mark() end
          ImGui.TreePop()
        end
      end
    end
    ...
    if ImGui.CollapsingHeader('Animations') then
      ...
      if cfg.anim.sheen then sliderf('  Sweep every', cfg.anim, 'sheenPeriod', 0.5, 10.0, '%.1f s') end
      check('Scroll stripes (Stripes texture)', cfg.anim, 'stripeScroll')
      if cfg.anim.stripeScroll then sliderf('  Scroll speed', cfg.anim, 'stripeSpeed', 5.0, 120.0, '%.0f px/s') end
      ...
      if cfg.anim.hpSmoothing then sliderf('  HP speed', cfg.anim, 'hpSpeed', 1.0, 30.0) end
      check('Fade in on appear', cfg.anim, 'fadeIn')
      if cfg.anim.fadeIn then sliderf('  Fade in time', cfg.anim, 'fadeInDur', 0.05, 2.0, '%.2f s') end
      check('Fade out on leave', cfg.anim, 'fadeOut')
      if cfg.anim.fadeOut then sliderf('  Fade out time', cfg.anim, 'fadeOutDur', 0.05, 2.0, '%.2f s') end
      ...
      if cfg.anim.appearPop then sliderf('  Pop time', cfg.anim, 'popDur', 0.05, 1.0, '%.2f s') end
